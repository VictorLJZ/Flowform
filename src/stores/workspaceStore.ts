    ...
    (set, get) => ({
      ...
      fetchWorkspaces: async () => {
        ...
        try {
          ...
          console.log('[WorkspaceStore] Current workspace from state:', currentWorkspace)
          
          // Validate current workspace against database results
          if (currentWorkspace) {
            ...
            if (!workspaceStillExists) {
              ...
              if (workspaces.length > 0) {
                console.log('[WorkspaceStore] Resetting to first available workspace:', workspaces[0])
                set({ currentWorkspace: workspaces[0] })
              } else {
                console.log('[WorkspaceStore] No workspaces available, clearing current workspace')
