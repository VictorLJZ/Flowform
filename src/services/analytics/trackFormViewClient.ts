...
/**
 * Track a form view event - Client-side implementation
 * Uses the API route to track form views
 * 
 ...
 */
export async function trackFormViewClient(
  formId: string,
  ...
  const trackWithRetry = async (retries = 2): Promise<void> => {
    try {
      ...
      if (retries > 0) {
        console.log(`Retrying form view tracking after error (${retries} retries left)`);
        return trackWithRetry(retries - 1);
      }
