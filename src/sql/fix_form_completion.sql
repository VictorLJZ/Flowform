-- This script fixes the track_form_completion function to match the actual database schema
-- Using the correct column names from the form_metrics table
...
BEGIN
  ...
  -- FIXED: Using the correct column names from the actual database schema
  ...
  VALUES (
    p_form_id,
    0, -- No change to total_views
    0, -- No change to unique_views
    0, -- No change to total_starts
    1, -- Increment total_completions by 1
    0, -- Will calculate completion_rate in the UPDATE
    v_completion_time_seconds,
    0, -- No change to bounce_rate
    v_timestamp
  )
  ON CONFLICT (form_id) 
  DO UPDATE SET 
    ...
    completion_rate = CASE 
                       WHEN form_metrics.total_starts > 0 
                       THEN (form_metrics.total_completions + 1)::numeric / form_metrics.total_starts
                       ELSE 0
                      END,
    -- Update average time as running average
    average_completion_time_seconds = CASE 
                            WHEN form_metrics.total_completions > 0 
                            THEN (form_metrics.average_completion_time_seconds * form_metrics.total_completions + v_completion_time_seconds) / (form_metrics.total_completions + 1)
